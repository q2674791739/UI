...
function SaveManager:IgnoreThemeSettings()
    self:SetIgnoreIndexes({
        "BackgroundColor", "MainColor", "AccentColor", "OutlineColor", "FontColor", -- 主题相关
...
function SaveManager:Load(name)
    ...
    if not success then return false, '解码失败' end
    ...
    for _, option in decoded.objects do
        ...
        task.spawn(self.Parser[option.type].Load, option.idx, option) -- 使用 task.spawn() 避免配置加载被阻塞。
    end
...
function SaveManager:LoadAutoloadConfig()
    ...
    if isfile(autoLoadPath) then
        local successRead, name = pcall(readfile, autoLoadPath)
        if not successRead then
            self.Library:Notify('加载自动加载配置失败：读取文件错误')
            return
        end
...
function SaveManager:DeleteAutoLoadConfig()
    ...
    function SaveManager:BuildConfigSection(tab)
        assert(self.Library, 'SaveManager:BuildConfigSection -> 必须先设置 SaveManager.Library')
        ...
        section:AddButton('创建配置', function()
            ...
            if name:gsub(' ', '') == '' then
                self.Library:Notify('配置名称无效（为空）', 2)
                return
            end
