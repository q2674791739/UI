...
do
    ...
    function CustomImageManager.AddAsset(AssetName: string, RobloxAssetId: number, URL: string, ForceRedownload: boolean?)
        ...
        assert(typeof(RobloxAssetId) == "number", "RobloxAssetId 必须是数字")
...
function Library:SetDPIScale(value: number) 
    assert(type(value) == "number", "DPI 缩放期望数字类型，但得到 " .. typeof(value))
...
do
    ...
    function BaseAddonsFuncs:AddKeyPicker(Idx, Info)
        ...
        assert(Info.Default, string.format("AddKeyPicker (IDX: %s): 缺少默认值。", tostring(Idx)))
        ...
        if KeyPicker.Mode == "Press" then
            assert(ParentObj.Type == "Label", "模式为 \"Press\" 的快捷键选择器只能应用于标签。")
...
function BaseAddonsFuncs:AddColorPicker(Idx, Info)
    local ParentObj = self
    local ToggleLabel = self.TextLabel
assert(Info.Default, string.format("AddColorPicker (IDX: %s): 缺少默认值。", tostring(Idx)))
...
local ColorPicker = {
    ...
    Type = "ColorPicker";
    Title = typeof(Info.Title) == "string" and Info.Title or "颜色选择器",
...
do
    ...
    ContextMenu:AddOption("复制 HEX", function()
        pcall(setclipboard, ColorPicker.Value:ToHex())
        Library:Notify("十六进制代码已复制到剪贴板！", 2)
    end)

    ContextMenu:AddOption("复制 RGB", function()
        pcall(setclipboard, table.concat({ math.floor(ColorPicker.Value.R * 255), math.floor(ColorPicker.Value.G * 255), math.floor(ColorPicker.Value.B * 255) }, ", "))
        Library:Notify("RGB 值已复制到剪贴板！", 2)
    end)
end
...
function BaseAddonsFuncs:AddDropdown(Idx, Info)
    ...
    assert(Info.Values, string.format("AddDropdown (IDX: %s): 缺少下拉选项列表。", tostring(Idx)))
    if not (Info.AllowNull or Info.Default) then
        Info.Default = 1
        warn(string.format("AddDropdown (IDX: %s): 缺少默认值，已选择第一项。如果这是有意的，请传入 `AllowNull` 为 true。", tostring(Idx)))
    end
...
function BaseGroupboxFuncs:AddLabel(...)
    ...
    if select(2, ...) ~= nil and typeof(select(2, ...)) == "table" then
        if select(1, ...) ~= nil then
            assert(typeof(select(1, ...)) == "string", "Idx 期望字符串类型，但得到 " .. typeof(select(1, ...)))
        end
...
function BaseGroupboxFuncs:AddButton(...)
    ...
    assert(typeof(Button.Func) == "function", "AddButton：缺少 `Func` 回调。")
...
function Button:AddButton(...)
    ...
    assert(typeof(SubButton.Func) == "function", "AddButton：缺少 `Func` 回调。")
...
function BaseGroupboxFuncs:AddInput(Idx, Info)
    assert(Info.Text, string.format("AddInput (IDX: %s): 缺少 `Text` 字符串。", tostring(Idx)))
...
function BaseGroupboxFuncs:AddSlider(Idx, Info)
    assert(Info.Default,    string.format("AddSlider (IDX: %s): 缺少默认值。", tostring(Idx)))
    assert(Info.Text,       string.format("AddSlider (IDX: %s): 缺少滑块文本。", tostring(Idx)))
    assert(Info.Min,        string.format("AddSlider (IDX: %s): 缺少最小值。", tostring(Idx)))
    assert(Info.Max,        string.format("AddSlider (IDX: %s): 缺少最大值。", tostring(Idx)))
    assert(Info.Rounding,   string.format("AddSlider (IDX: %s): 缺少小数位。", tostring(Idx)))
...
function BaseGroupboxFuncs:AddImage(Idx, Info)
    ...
    assert(Icon, "图片必须是有效的 Roblox 资源、有效的 URL 或有效的 lucide 图标。")
    ...
    function Image:SetImage(NewImage: string)
        assert(typeof(NewImage) == "string", "图片必须是字符串。")
    ...
    function Image:SetColor(Color: Color3)
        assert(typeof(Color) == "Color3", "颜色必须是 Color3 值。")
    ...
    function Image:SetRectOffset(RectOffset: Vector2)
        assert(typeof(RectOffset) == "Vector2", "RectOffset 必须是 Vector2 值。")
    ...
    function Image:SetRectSize(RectSize: Vector2)
        assert(typeof(RectSize) == "Vector2", "RectSize 必须是 Vector2 值。")
    ...
    function Image:SetScaleType(ScaleType: Enum.ScaleType)
        assert(
            ...
            "ScaleType 必须是有效的 Enum.ScaleType。"
        )
    ...
    function Image:SetTransparency(Transparency: number)
        assert(typeof(Transparency) == "number", "透明度必须是 0 到 1 之间的数字。")
        assert(Transparency >= 0 and Transparency <= 1, "透明度必须在 0 到 1 之间。")
...
function BaseGroupboxFuncs:AddVideo(Idx, Info)
    ...
    function Video:SetLooped(Looped: boolean)
        assert(typeof(Looped) == "boolean", "Looped 必须是布尔值。")
    ...
    function Video:SetVolume(Volume: number)
        assert(typeof(Volume) == "number", "音量必须是 0 到 10 之间的数字。")
    ...
    function Video:SetPlaying(Playing: boolean)
        assert(typeof(Playing) == "boolean", "Playing 必须是布尔值。")
...
function BaseGroupboxFuncs:AddUIPassthrough(Idx, Info)
    ...
    assert(Info.Instance, "必须提供 Instance。")
    assert(
        typeof(Info.Instance) == "Instance" and Info.Instance:IsA("GuiBase2d"),
        "Instance 必须继承自 GuiBase2d。"
    )
    assert(typeof(Info.Height) == "number" and Info.Height > 0, "高度必须是大于 0 的数字。")
    ...
    function Passthrough:SetHeight(Height: number)
        assert(typeof(Height) == "number" and Height > 0, "高度必须是大于 0 的数字。")
...
function BaseGroupboxFuncs:AddDependencyBox()
    ...
    function Depbox:SetupDependencies(Dependencies)
        for _, Dependency in next, Dependencies do
            assert(typeof(Dependency) == "table", "SetupDependencies：依赖项不是 `table` 类型。")
            assert(Dependency[1], "SetupDependencies：依赖项缺少元素参数。")
            assert(Dependency[2] ~= nil, "SetupDependencies：依赖项缺少值参数。")
        end
    ...
    function BaseGroupboxFuncs:AddDependencyGroupbox()
        ...
        function DepGroupbox:SetupDependencies(Dependencies)
            for _, Dependency in pairs(Dependencies) do
                assert(typeof(Dependency) == "table", "依赖项应为 table。")
...
function Window:AddDialog(Idx, Info)
    assert(Info.Title, "AddDialog：缺少 `Title` 字符串。")
    assert(Info.Description, "AddDialog：缺少 `Description` 字符串。")
